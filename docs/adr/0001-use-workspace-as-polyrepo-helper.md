...
- 실제 로직은 macOS와 Windows Git Bash에서 실행 가능한 Bash CLI인 `scripts/workspace.sh`에 둔다.
- Taskfile은 `scripts/workspace.sh`를 호출하는 얇은 공식 진입점만 제공한다.
- Windows 사용자는 Git Bash 터미널에서 실행한다.
- 신규 사용자는 `workspace`만 clone한 뒤 `task help`, `task bootstrap`, `task status`, `task list`, `task doctor`로 작업공간을 준비한다.
...
- Node, Go, Python 같은 추가 런타임 없이 Git Bash와 Task만 설치하면 같은 명령 UX를 쓸 수 있다.
