...
```text
외부 Ingress로 들어온 요청이 Kong과 서비스까지 도달하는가?
...
aws-dev 기본 실행 경로는 클러스터 내부 CronJob에서 외부 Ingress DNS를 호출하는 구조다.
...
aws-dev의 credential source는 Kubernetes Secret이다.
...
aws-dev는 외부 Ingress DNS를 기본 대상으로 하며 5-10분 주기를 후보로 둔다.
...
- aws-dev에서는 외부 Ingress DNS 기준 smoke 또는 full journey가 반복 실행된다.
