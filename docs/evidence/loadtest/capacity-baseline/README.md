...
각 서비스 단계는 기본적으로 같은 RPS 계단을 사용한다. 예를 들어 `10 -> 20 -> 40 -> 80 -> 160 RPS`처럼 올리고, 각 계단마다 p95, p99, error rate, CPU usage, CPU throttling을 기록한다. 최종 최대 RPS는 임시 SL을 만족한 마지막 계단이다.

auth-service는 로그인 API 특성상 별도 기준을 둔다. `POST /auth/login`은 password verify 비용이 포함된 CPU-bound 경로이고, 일반 조회 API처럼 반복 호출되는 endpoint가 아니다. 따라서 capacity baseline에서는 `30 RPS`와 `40 RPS`를 기대 처리량 후보로 두고, 이 구간의 p95/p99와 CPU 사용량을 먼저 확인한다.
...
| 서비스 | 임시 p95 기준 |
| --- | ---: |
| auth-service | `30 RPS: 100-150ms`, `40 RPS: 150-250ms` |
