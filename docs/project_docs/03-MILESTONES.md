...
- 통합 검증 시나리오의 공통 실행 계약은 특정 service repo가 아니라 `gitops`의 검증 영역에 두는 방향을 우선 검토한다.
...
| --- | --- | --- | --- | --- |
| Phase 0 | 2026-05-27 ~ 2026-06-02 | 계획과 계약 확정 | 무엇을 만들고 무엇을 검증할 것인가? | PRD, 아키텍처, API/event 계약, P0 시나리오, 검증 자동화 소유 경계, 역할이 정리된다. |
...
- 팀원이 시나리오를 폴더 단위로 나누어 작업할 수 있도록 검증 계약과 소유 repo가 정리됐는가?
...
- 데이터 모델 초안
- P0 검증 시나리오
- 검증 자동화 소유 repo와 폴더 배치 원칙
- 시나리오 폴더 단위 협업 규칙
- suite/scenario/report 최소 계약
- 역할과 소유 영역
...
- 공통 검증 시나리오를 어디에 두고 어떻게 분산 작업할지 합의했다.
...
- 좌석 conflict 테스트 결과
- 정상 예매 E2E 절차
- P0 suite와 정상 예매 scenario skeleton
...
- GitHub Actions 또는 Testkube 기반 검증 실행 skeleton
- 중간 발표용 진행 요약
...
- `docs/project_docs/05-epics/E12-validation-automation.md`를 기준으로 검증 자동화 소유 경계와 선행 작업을 이슈로 나눈다.
