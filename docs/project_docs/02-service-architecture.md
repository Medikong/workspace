...
공연 운영자는 공연을 실제로 공급하는 주최사와 공연장 측을 합친 액터다. 이 유스케이스는 고객 예매 흐름 이전에 운영자가 공연 상품, 회차, 좌석, 판매 조건을 준비하고 판매 이후 현황과 정산 기준 데이터를 확인하는 관점으로 둔다.
...
플랫폼 운영자는 예매 서비스 전체 품질과 판매 통제를 책임지는 액터다. 이 유스케이스는 공연 운영자가 제출한 상품과 판매 조건을 검수하고, 오픈 당일의 판매 상태와 트래픽 대응을 통제하는 관점으로 둔다.
...
플랫폼 운영자, 공연 운영자, 사용자는 모두 `dashboard`에서 시작하지만 서로 다른 API 경로를 사용한다. 사용자는 공개 예매 API를 호출하고, 공연 운영자는 `/provider/...`, 플랫폼 운영자는 `/admin/...` 경로로 운영 API를 호출한다. 서비스 간 즉시 처리는 REST로 두고, 예약/결제/티켓/운영 이력 후속 처리는 Kafka 이벤트로 분리한다.

```mermaid
flowchart LR
    User["사용자"] --> UserUI["dashboard\n사용자 예매 화면"]
    ShowOperator["공연 운영자"] --> ProviderUI["dashboard\n/provider 운영 화면"]
    PlatformOperator["플랫폼 운영자"] --> AdminUI["dashboard\n/admin 운영 화면"]

    UserUI --> Auth["auth-service\n로그인/JWT"]
    ProviderUI --> Auth
    AdminUI --> Auth

    UserUI --> Concert["concert-service\n공연/회차/좌석 조회"]
    UserUI --> Reservation["reservation-service\n좌석 lock/예약/취소"]
    UserUI --> Payment["payment-service\n결제 mock"]
    UserUI --> Ticket["ticket-service\n티켓 조회"]
    UserUI --> Notification["notification-service\n내 알림"]

    ProviderUI --> Concert
    ProviderUI --> Reservation
    ProviderUI --> Payment
    ProviderUI --> Notification
    ...
    Reservation -->|reservation-created / reservation-expired| Kafka["Kafka topics"]
    ...
    Kafka -->|reservation / payment / ticket / operation events| Notification
    Ticket --> S3["S3\nQR/PDF 저장"]
```
...
API는 액터 관점이 섞이지 않도록 사용자, 공연 운영자, 플랫폼 운영자 기준으로 그룹핑한다. 사용자 예매 API는 공개 예매 흐름의 resource path를 사용하고, 공연 운영자 화면은 `/provider/...`, 플랫폼 운영자 화면은 `/admin/...` prefix를 사용한다. 역할 구분은 `auth-service`가 발급한 JWT role claim으로 처리한다.
...
| `concert-approved` | `concert-service` | `notification-service`, analytics 후보 | 공연 상품 승인 이력과 공연 운영자 알림 |
