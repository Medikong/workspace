...
| done | aws-dev synthetic CronJob의 기본 호출 주소를 internal Kong service로 전환 | gitops | `platform/synthetic/values/aws-dev.yaml` |
...
HTTP 도달성 문제도 `platform/synthetic/values/aws-dev.yaml`에서 aws-dev CronJob의 호출 주소를 내부 Kong service로 전환해 해결했다.

- 변경 후 `synthetic-traffic-aws-dev` Argo Application은 revision `fee6601f2725e882eaa69ade4b4de0594fb9630c`로 `Synced / Healthy` 상태다.
- 수동 Job `synthetic-traffic-manual-20260612054119`는 `Complete 1/1`로 종료됐다.
- Pod `synthetic-traffic-manual-20260612054119-tnjqr`는 `phase=Succeeded`, `reason=Completed`, `exitCode=0`으로 끝났다.
- k6 로그에는 `synthetic_run_finished`가 남았고, `auth.login`, catalog, reservation, payment, ticket, notification 단계가 모두 2xx로 통과했다.
